...
{	
	...
	$("#s_city").removeOption(/./);
	$("#s_town").removeOption(/./);
		var items = eval("city"+id.substring(id.indexOf(",")+1,id.length));
		var len = items.length;
		var html = '';
		$("#s_city").addOption("0", "不限");
		var str ='';
		for(var i=0;i<len;i++){
			$("#s_city").addOption(items[i].name + ","+ items[i].code, items[i].name,false);
			str+=(","+items[i].code);
		}
		var f = items[0].code;
...
$.ajax({
			url: 'townlist.htm',
			type: 'GET',
			data: 'cityId='+id.substring(id.indexOf(",")+1,id.length),
			dataType: 'json',
			timeout: 10000,
		    async:false,
		   ...
		   for(var i=0;i<len;i++){
			html=html+'<option value="'+items[i].name + ","+ items[i].code+'">'+items[i].name+'</option>';
		   }
		$("#town_id").html(html);
			}
		});
...
function initcity(p_1_v,p_2_v){
	...
	{$("#s_province").addOption(Provinces[i].name + ","+ Provinces[i].code, Provinces[i].name,false);}
