...
public class ModuleDirective extends Directive {
	...
	public boolean render(InternalContextAdapter icad, Writer writer, Node node) throws IOException,
		...
		try {
			String moduleAction = getActionName(params.get("moduleName"));
			String method = getMethod(params.get("moduleName"));
			ModuleAction action = (ModuleAction) SpringContextHolder.getBean(moduleAction);
			// 处理参数
			...
			if (method.equals("head")) {
				action.head(icad, params, context);
			} else if (method.equals("foot")) {
				action.foot(icad, params, context);
			}
	...
	private String getMethod(String moduleName) {
